#!/usr/bin/env python3
"""
Gemini 전체 PSUR 보고서 생성 테스트
- 15개 섹션 전체 작성
- gemini-3-flash-preview, gemini-2.5-pro 모델 비교
"""
...
def call_gemini_api(model, prompt, max_output_tokens=65536):
    """Gemini API 호출"""
    if not API_KEY:
...
def main():
    print("=" * 80)
    print("Gemini 전체 PSUR 보고서 생성 테스트")
    print("=" * 80)
    ...
    # 데이터 정의서
    rawdata_file = BASE_PATH / "01_Context" / "RawData_Definition.md"
    rawdata_content = read_file(rawdata_file)
    ...
    raw_data_content = read_file(raw_data_file)
    print(f"  - RAW Data (total_MD.md): {len(raw_data_content):,} 문자")

    # 사용자 입력
    user_input_file = BASE_PATH / "04_MainDocuement" / "test_UserInput.md"
    user_input_content = read_file(user_input_file)
...
"""
    ...
    # 결과 요약 저장
    summary_file = output_dir / f"full_report_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print("결과 요약")
    print(f"{'='*80}")
    print(f"{'모델':<25} {'상태':<6} {'시간(분)':<10} {'입력토큰':<12} {'출력토큰':<12} {'비용($)':<10}")
    print("-" * 80)

    for r in results:
        if r["success"]:
            print(f"{r['model']:<25} {'성공':<6} {r['duration_minutes']:<10.2f} {r['input_tokens']:<12,} {r['output_tokens']:<12,} {r['cost_usd']:<10.4f}")
        else:
            print(f"{r['model']:<25} {'실패':<6} {r.get('duration_seconds', 0)/60:<10.2f} {'N/A':<12} {'N/A':<12} {'N/A':<10}")
