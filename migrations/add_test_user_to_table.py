...
def main():
    print("\n" + "="*60)
    print("  users 테이블에 테스트 사용자 추가")
    print("="*60)

    # Read SQL file
    sql_file = os.path.join(os.path.dirname(__file__), 'add_test_user_to_table.sql')

    with open(sql_file, 'r', encoding='utf-8') as f:
        sql_content = f.read()

    print(f"\n📄 SQL 파일: {sql_file}")
    print(f"📧 사용자: author@kpsur.test")
    print(f"👤 역할: Author")
    print(f"\n🚀 SQL 실행 중...\n")

    if execute_sql(sql_content):
        print("\n✅ 테스트 사용자가 users 테이블에 추가되었습니다.")
    else:
        print("\n❌ 실패했습니다.")
